...
def interp_2D_grids(xx_in, yy_in, zz_in, xx_out, yy_out, resol_grid_in, resol_grid_out, option_flag):
    ...
    for loop_ctr in np.arange(0, np.size(out_flat, 0), 1):
        ...
        out_flat_values[loop_ctr, 0] = compute_value_grid_cells_selected(curr_lon, curr_lat, resol_grid_in,
                                                                         resol_grid_out, points_selected, option_flag)
        print(loop_ctr)
...
def compute_value_grid_cells_selected(curr_lon, curr_lat, resol_grid_in, resol_grid_out, points_selected, option_flag):
    ...
    for loop_ctr in np.arange(0, np.size(points_selected, 0), 1):
        ...
        if (np.logical_and(resol_grid_in / 2 < resol_grid_out / 2,
                           distance_points_lon + resol_grid_in / 2 < resol_grid_out / 2)):
        ...
        elif (np.logical_and(resol_grid_out / 2 < resol_grid_in / 2,
                             distance_points_lon + resol_grid_out / 2 < resol_grid_in / 2)):
            ...
            #        % du point de grille de sortie
        if (np.logical_and(resol_grid_in / 2 < resol_grid_out / 2,
                           distance_points_lat + resol_grid_in / 2 < resol_grid_out / 2)):
            lat_a_considerer = resol_grid_in;
            #            % si la zone du point de grille de sortie est à l'interieur de la zone
            #            % du point de grille d'entrée
        elif (np.logical_and(resol_grid_out / 2 < resol_grid_in / 2,
                             distance_points_lat + resol_grid_out / 2 < resol_grid_in / 2)):
...
# array_cmap = np.loadtxt("C:\\Users\\Damien Roques\\Documents\\technical
# stuff\\WinPython-64bit-3.3.5.0\\python-3.3.5.amd64\\Scripts\\mapping_tool\\country_map\\glbnds.asc",skiprows=6);
#
# np.save("./country_map/array_cmap", array_cmap)


# array_pop = np.loadtxt("C:\\Users\\Damien Roques\\Documents\\technical
# stuff\\WinPython-64bit-3.3.5.0\\python-3.3.5.amd64\\Scripts\\mapping_tool\\pop_count\\glp15ag.asc",skiprows=6);
#
