...
def compute_ecef_2_sc_nominal(vect_ecef, nadir_ecef, sat_pos_ecef, normal_vector):
    ...
    mat_ecef2sat = compute_ecef_2_sc_rot_matrix(nadir_ecef, sat_pos_ecef, normal_vector) # rotation matrix to go to SAT coordinate system
...
#        points_sat_coord = np.dot(mat_ecef2sat, (vect_ecef.T - sat_pos_ecef.T).T) #TODO : change the ugly tranpose (this is to handle 1D vectors)
#
#        
#    else:
#    
#        # matrix calculation using np.einsum
#        #( replaces code found on Internet : https://jameshensman.wordpress.com/2010/06/14/multiple-matrix-multiplication-in-numpy/)
...
#    
#    # compute matrix to go to SC coord. system
#    mat_sat = compute_ecef_2_sc_rot_matrix(nadir_ecef, sat_pos_ecef, normal_vector) # rotation matrix to go to SAT coordinate system
#
#    # case with 1D vectors
...
#<><><><><><><><><><><><> Consistency checks <><><><><><><><><><><><><><><><>
    ...
    if np.logical_or(np.size(inclination_angle) != np.size(nadir_ecef,1), np.size(inclination_angle) != np.size(flag_asc_desc)):
