...
#---------------------------------------------------------------------------    
    ...
    vect[:,flag_asc_desc == 'ASC'] = np.array([-nadir_ecef[1],nadir_ecef[0],np.zeros_like(nadir_ecef[0])])[:,flag_asc_desc == 'ASC']
    vect[:,flag_asc_desc == 'DESC'] = np.array([nadir_ecef[1],-nadir_ecef[0],np.zeros_like(nadir_ecef[0])])[:,flag_asc_desc == 'DESC']
...
#    vect_result = np.atleast_2d(vect/norm_vect).T
    ...
    for inc in inclination_angle:
        ...
        mat_rotation = np.array([[  np.cos(inc) + ux**2*(1-np.cos(inc))      ,   ux*uy*(1-np.cos(inc)) - uz*np.sin(inc)     ,   ux*uz*(1-np.cos(inc))+uy*np.sin(inc)  ], \
                                 [  ux*uy*(1-np.cos(inc)) + uz*np.sin(inc)    ,   np.cos(inc) + uy**2*(1-np.cos(inc))        ,   uy*uz*(1-np.cos(inc)) - ux*np.sin(inc)   ], \
                                 [    ux*uz*(1-np.cos(inc)) - uy*np.sin(inc)  ,   uy*uz*(1-np.cos(inc)) + ux*np.sin(inc)     ,   np.cos(inc) + uz**2*(1-np.cos(inc))  ]])
...
def compute_ecef_2_sc_nominal(vect_ecef, nadir_ecef, sat_pos_ecef, normal_vector):
    ...
    mat_sat = compute_sc_coord_vectors(nadir_ecef, sat_pos_ecef, normal_vector) # rotation matrix to go to SAT coordinate system
    ...
    if sat_pos_ecef.ndim == 1:
        ...
        # matrix calculation (smartly done with code found on Internet : https://jameshensman.wordpress.com/2010/06/14/multiple-matrix-multiplication-in-numpy/)
...
def compute_elev_grd(pos_station, pos_sat):
    ...
    cos_angle = np.atleast_1d(np.sum(pos_station * vect_dir_sat, axis=0)/(np.sum(pos_station*pos_station,axis=0)**0.5 * \
...
def compute_intersection_WGS84_ecef(vectors, sat_pos):
    ...
    C = ((sat_pos[0]**2 + sat_pos[1]**2)/RTE_2 + sat_pos[2]**2/RTP_2 -1)   #*np.ones((np.size(ecef_points,0),1))
    ...
    return np.array([vectors[0]*result+sat_pos[0], vectors[1]*result+sat_pos[1], vectors[2]*result+sat_pos[2]])
...
#     first size arrays as 2D arrays    
    [vect_ecef, nadir_ecef, normal_vector, sat_pos_ecef] = resize_arrays([vect_ecef, nadir_ecef, normal_vector, sat_pos_ecef])
...
#        points_sat_coord = np.sum(np.transpose(mat_to_use,(0,2,1)).reshape(np.size(nadir_ecef,1),3,3,1) * vect_ecef_bis.reshape(np.size(nadir_ecef,1),3,1,1),-3)
...
def compute_az_elev_to_ecef(az_elev, \
    ...
    if nadir_ecef.ndim == 1:
        ...
        points_ecef_unit = np.sum(np.transpose(mat_to_use,(0,2,1)).reshape(np.size(nadir_ecef,1),3,3,1) * vect_uvw_untilted.reshape(np.size(nadir_ecef,1),3,1,1),-3)
...
def translate_az_elev_sc_to_az_elev_ant(az_elev_beam_centers, az_elev_points):    
    ...
    set of coordinates, which is the satellite nadir in the case of SC, to the center of the beam (which is the center of
...
def translate_az_elev_ant_to_az_elev_sc(az_elev_beam_centers, az_elev_points):    
    ''' This function switches from (A,B)ANT to (A,B)SC coordinate system. see opposite function (translate_az_elev_sc_to_az_elev_ant)
...
def compute_roll_pitch_yaw(roll, pitch, yaw):
    ...
    first_col = np.array([np.cos(yaw) * np.cos(pitch), np.cos(yaw) * np.sin(pitch) * np.sin(roll) - np.sin(yaw) * np.cos(roll), np.cos(yaw) * np.sin(pitch) * np.cos(roll) + np.sin(yaw) * np.sin(roll)])
    
    second_col = np.array([np.sin(yaw) * np.cos(pitch), np.sin(yaw) * np.cos(pitch) * np.sin(roll) + np.cos(yaw) * np.cos(roll), np.sin(yaw) * np.sin(pitch) * np.cos(roll) - np.cos(yaw) * np.sin(roll)])
