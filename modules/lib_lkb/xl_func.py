...
#    ws = xlwings.Sheet(worksheet_name)
    ...
    for field_item in fields_list:
        counter_col = 1
        col_list = xlwings.sheets[worksheet_name].range((1,1),(1,100)).value    #TODO: 100 columns limit is completely arbitrary. to replace with smthg smarter    
...
#                xlwings.Range(ws, (1,counter_col)).color = field_color
                ...
                xlwings.sheets[worksheet_name].range((2,counter_col), (nb_elts+1, counter_col)).value = np.atleast_2d(perfo_dict[field_item]).T
...
def find_format_to_col(field_item, tab_name):
    ''' This function applies a given format to a column for saving in excel (i.e. color of the cell, and unit)
        if the field is not found in the normal tab but in on the others, then it is a joined value and written in grey, using also the same format as described in the original tab
    ...
    col_list = xlwings.Range(ws1, (1,1),(1,100)).value    #TODO: 100 columns limit is completely arbitrary. to replace with smthg smarter    
    counter = 1    
