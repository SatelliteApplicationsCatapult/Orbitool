...
dbTest.define_table('testUpXcel',
                    ...
                    Field('fileUp','upload',requires=IS_NOT_EMPTY(),autodelete=True)) ##This is a good example to build the real excel database from
...
dbLinkBudget.define_table('Earth_Cord_GW',
...
dbLinkBudget.define_table('Job',
                          ...
                          Field('file_up','upload',requires=IS_NOT_EMPTY(),label='File'),
                          Field('simulator_mode','string',requires=IS_IN_SET(('FWD','RTN')),default='FWD',label='Simulation Mode'),
                          Field('sat_geo_params','boolean',label='Compute Satellite Geometric Parameters'),
                          Field('points2trsp','boolean',label='Assign Earth points coverage to transponder(VSAT)'),
                          Field('gw2trsp','boolean',label='Assgin Earth gateways to transponder (Gateway)'),
                          Field('comp_point_cover','boolean',label='Compute geometric parameters for Earth points coverage (VSAT)'),
                          Field('comp_gw_cover','boolean',label='Compute geometric parameters for Earth gateways (VSAT)'),
                          Field('propa_feeder_link','boolean',label='Compute propagation on Feeder Link (Gateway)'),
                          Field('propa_user_link','boolean',label='Compute propagation on User Link (VSAT)'),
                          Field('sat_up_perf','boolean',label='Compute satellite uplink performances (e.g. G/T)'),
                          Field('sat_dwn_perf','boolean',label='Compute satellite downlink performances (e.g. EIRP)'),
                          Field('comp_link_budget','boolean',label='Compute link budget'),)
